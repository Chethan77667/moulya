...
class LecturerService:
    ...
    @staticmethod
    def reset_marks_for_assessment(subject_id, lecturer_id, assessment_type):
        """Reset all marks for a given assessment type for this subject by this lecturer.

        Removes all `StudentMarks` rows matching subject, lecturer and assessment_type.
        """
        try:
            # Verify lecturer is assigned to this subject
            assignment = SubjectAssignment.query.filter_by(
            ...
            # Delete matching marks entries
            (StudentMarks.query
                .filter(
                    StudentMarks.subject_id == subject_id,
                    StudentMarks.lecturer_id == lecturer_id,
                    StudentMarks.assessment_type == assessment_type
                )
                .delete(synchronize_session=False))

            db.session.commit()
            return True, f"Reset marks for assessment '{assessment_type}'."
        except Exception as e:
            db.session.rollback()
            return False, f"Error resetting marks: {str(e)}"

    @staticmethod
    def reset_month_attendance(subject_id, lecturer_id, month, year):
        """Reset monthly attendance (summary and per-student) for a month/year for this lecturer.

        Deletes `MonthlyStudentAttendance` rows and the `MonthlyAttendanceSummary` row for the month.
        Also removes any `AttendanceRecord` daily rows for that month for safety.
        """
        try:
            # Verify lecturer is assigned to this subject
            assignment = SubjectAssignment.query.filter_by(
            ...
            # Delete per-student monthly attendance rows
            (MonthlyStudentAttendance.query
                .filter(
                    MonthlyStudentAttendance.subject_id == subject_id,
                    MonthlyStudentAttendance.lecturer_id == lecturer_id,
                    MonthlyStudentAttendance.month == month,
                    MonthlyStudentAttendance.year == year
                )
                .delete(synchronize_session=False))

            # Delete monthly summary for this lecturer/month
            (MonthlyAttendanceSummary.query
                .filter(
                    MonthlyAttendanceSummary.subject_id == subject_id,
                    MonthlyAttendanceSummary.lecturer_id == lecturer_id,
            ...
            # Best-effort cleanup of daily attendance records in that month
            ...
            db.session.commit()
            return True, f"Reset attendance for {month}/{year}."
        except Exception as e:
            db.session.rollback()
            return False, f"Error resetting monthly attendance: {str(e)}"